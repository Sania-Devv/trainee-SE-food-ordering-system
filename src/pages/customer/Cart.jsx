...
export default function Cart() {
  ...
  if (loading) {
    return <h2 className="text-center py-10">Loading cart...</h2>;
  }
  ...
  const handleDelete = (itemId) => {
    dispatch(deleteCartItem(itemId)).then(() => dispatch(fetchCart()));
  };
          ...
          <div className="xl:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
              ...
              {(!items || items.length === 0) && (
                <p className="px-6 py-6 text-gray-500">Your cart is empty.</p>
              )}

              {items?.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col md:flex-row gap-5 px-4 sm:px-6 py-6 border-b border-gray-100"
                >
                  {/* Image */}
                  <img
                    src={
                      ...
                      <div>
                        <span
                          className={`inline-block text-xs font-semibold px-2 py-0.5 rounded-full mb-2 ${
                            item.type === "deal"
                              ? "bg-orange-100 text-[#FC8A06]"
                              : "bg-blue-100 text-blue-600"
                          }`}
                        >
                          {item.type === "deal" ? "Deal" : "Menu Item"}
                        </span>
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className="w-8 h-8 rounded-full border text-lg flex items-center justify-center hover:bg-gray-100"
                        >
                          -
                        </button>
